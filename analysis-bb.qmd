...
```{r Creating individual matrices}
   ...
   bb <- function(x) {
      x <- suppressMessages(disparity(x, alpha = 0.25, narrative = FALSE))
      return(x)
      }
...
```{r CA Statistics}
   ...
   png(file = here("Plots", "scree-genres.png"), width=300, height=600)
   fviz_eig(CA.r, ncp = 6) + stuff + ggtitle("") + ylim(0, 12) + 
      labs(y = "Explained Var. %")
   dev.off()
   png(file = here("Plots", "scree-labels.png"), width=300, height=600)
...
```{r Data Examples}
   ...
   for (id in c("id_65", "id_236", "id_1114", "id_2002")) {
      ...
      png(file = here("Plots", plot.name), width=700, height=700)
...
```{r Label Cloud Plots}
   ...
   png(file = here("Plots", "label-proto-ex1.png"), width=w, height=h)
      make.fuzz.plot(row.lab.dat, sup.lab.dat, bns = 6, ls = 6,
...
```{r Genre and Label Subcloud Variance}
   w <- 600
   h <- 700
   var.g <- make.ridge.dat(sup.gen.dat, row.gen.dat) %>% 
   ...
   var.l <- make.ridge.dat(sup.lab.dat, row.lab.dat) %>% 
      mutate(dist.cent.1 = (Dim.1 - Prot.1)^2) %>% 
      mutate(dist.cent.2 = (Dim.2 - Prot.2)^2) %>% 
      mutate(dist.cent.3 = (Dim.3 - Prot.3)^2) %>% 
      mutate(var = dist.cent.1 + dist.cent.2 + dist.cent.3) %>% 
   ...
   make.bar.plot(var.g) + xlim(0.75, 1.75)
   dev.off()
   png(file = here("Plots", "label-var.png"), width=w, height=h)
   make.bar.plot(var.l) + xlim(0.75, 1.75)
   dev.off()
```
...
```{r}
   ...
   classical.cloud <- filter(row.gen.dat.id, col.obj == "Classical")
   classical.dat <- left_join(classical.cloud, row.gen.dat.id)
   classical.dat <- left_join(classical.dat, demog.dat)
```

```{r}
   p <- ggplot(data = classical.dat, aes(Dim.1, Dim.2))
   p <- p + stat_density_2d(geom = "polygon", 
                            aes(alpha=after_stat(level)), bins = 5)
   p
```
