...
```{r setup, include=FALSE}
   ...
   source(here("Functions", "make.fuzz.plot.R"))
   source(here("Functions", "make.demog.plot.R"))
   source(here("Functions", "demog.dat.R"))
   source(here("Functions", "sdsm.back.R"))
```
...
```{r Summary Table of Averaged Perceptions}
   ...
   corrplot(tab, is.corr = FALSE, method = "color", 
      ...
      addCoef.col = "black", cl.pos = "n", col.lim = c(0, max(tab)),
...
```{r Creating individual matrices}
   ...
   bb.sdsm <- function(x) {
     tryCatch({
         w <- round(sdsm(x, alpha = NULL, missing.as.zero = TRUE)$Pupper, 2)
         return(w)
   ...
   ind.row <- lapply(ind.mat, function(x) {x %*% t(x)}) # individual genre projection
   ind.col <- lapply(ind.mat, function(x) {t(x) %*% x}) # individual label projection
```
   ...
   # row.bb <- lapply(row.bb, function(x) {ifelse(is.nan(x), 1, x)})
   # col.bb <- lapply(col.bb, function(x) {ifelse(is.nan(x), 1, x)})
   # row.bb <- lapply(row.bb, function(x) {diag(x) <- 1; x})
   # col.bb <- lapply(col.bb, function(x) {diag(x) <- 1; x})
   # save(row.bb, col.bb, file = here("bb.RData"))
   load(here("bb.RData"))
   p.val <- 0.45
   row.bb <- lapply(row.bb, function(x) {(x < p.val) * 1})
   col.bb <- lapply(col.bb, function(x) {(x < p.val) * 1})
...
```{r Genre Cloud Plots}
   ...
   png(file = here("Plots", "genre-proto-ex4.png"), width=w, height=h)
      make.fuzz.plot(row.gen.dat, col.gen.dat, sup.gen.dat,
                     seed = 125, ls = 5,
                     1, 2, pa = .35,  t = 2, b = 2, l = 2, r = 3.25, bns = 6,
   ...
   png(file = here("Plots", "genre-proto-ex6.png"), width=w, height=h)
      make.fuzz.plot(row.gen.dat, col.gen.dat, sup.gen.dat,
                     seed = 125, ls = 5,
                     1, 2, pa = .35,  t = 2, b = 2, l = 2, r = 3, bns = 6,
                     c("Alt_Rock", "Contemp_Rock", "Classic_Rock", "Metal", "Broadway"))
   dev.off()
...
```{r Label Cloud Plots}
   ...
   png(file = here("Plots", "label-proto-ex1.png"), width=w, height=h)
      make.fuzz.plot(row.lab.dat, col.lab.dat, sup.lab.dat, bns = 8, ls = 6,
                     1, 2, pa = .35,  t = 2.5, b = 2, l = 1.5, r = 2.15,
   ...
   png(file = here("Plots", "label-proto-ex2.png"), width=w, height=h)
      make.fuzz.plot(row.lab.dat, col.lab.dat, sup.lab.dat, bns = 6, ls = 6,
                     1, 2, pa = .35,  t = 2.2, b = 2.2, l = 1.5, r = 2.5,
                     c("Old", "Mid_Age", "Young"))
   dev.off()
